...
class VitaParser {
    ...
    private function _getContent($link) {
        if (file_exists($this->_cache_dir . 'vita_' . date('dmY'))) {
            $fp = fopen($this->_cache_dir . 'vita_' . date('dmY'), 'r');
            $get = fread($fp, filesize($this->_cache_dir . 'vita_' . date('dmY')));
            fclose($fp);
//            echo 'cache@@@@';
        } else {
            $get = file_get_contents("$link");
            if (strlen($get) > 10) { //Если страница доступна
                $fp = fopen($this->_cache_dir . 'vita_' . date('dmY'), 'w');
                fwrite($fp, $get);
                fclose($fp);
            } else {
